...
def parse_args() -> Namespace:
    ...
    parser.add_argument('--output_dir', type=str, default=None, help="Directory to save evaluation outputs. If none, will mirror model path in _results")
...
if __name__ == "__main__":
    ...
    print(f"Evaluating on: {', '.join(config.id() for config in dataset_configs)}")
    ...
    if not args.no_save:
        ...
        if args.output_dir is None:
            if args.model_type == 'base':
                output_dir = os.path.join("_results", "base", args.model_path.replace('/', '_'))
            else:
                output_dir = args.model_path.replace('_models', '_results')
        else:
        ...
        evaluator.save_results(output_dir, metadata=metadata)
